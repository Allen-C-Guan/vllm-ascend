...
| Name | Type | Default | Description |
| ---- | ---- | ------- | ----------- |
| `compile_backend` | string | `"auto"` | Compile backend track: `"auto"` keeps the legacy `enable_npugraph_ex` inference; `"fusion_pass"` / `"npugraph_ex"` pin one legacy track; `"inductor"` enables the inductor track (upstream Inductor compile_fx + torch_npu `triton_experimental`). When `cudagraph_mode` is not set, the track follows the `-O` presets exactly like upstream (`-O1` → PIECEWISE, `-O2`/`-O3` → FULL_AND_PIECEWISE, so uniform decode batches get the full-graph path by default); explicit `NONE` (compile-only), `PIECEWISE` and the full-graph family `FULL` / `FULL_AND_PIECEWISE` / `FULL_DECODE_ONLY` (upstream-aligned: `FULL`/`FULL_DECODE_ONLY` compile one single graph) are honored as-is. Moving from the previous track default (PIECEWISE) changes the compile cache key — existing track users get one cold recompile. Incompatible with `enforce_eager=True`, `-O0`, `VLLM_USE_STANDALONE_COMPILE=1`, `VLLM_USE_MEGA_AOT_ARTIFACT=1` and an explicit `enable_npugraph_ex=True` (all raise at config time). `VLLM_USE_BREAKABLE_CUDAGRAPH=1` wins over the track with a warning (upstream semantics: compilation mode is forced to NONE and the track is inert; set it to `0` to use the track). |
...
| `fuse_norm_quant`  | bool | `True` | Whether to enable fuse_norm_quant pass. On the inductor track these `fuse_*` flags chain the Ascend fusion passes into the track's post-grad pass manager (same conditions as the legacy track: 310P skips `fuse_norm_quant`/`fuse_muls_add`; W4A4 models skip `fuse_norm_quant`). These are the `ascend_compilation_config` flags — upstream `pass_config` flags of the same names stay off on the track. |
